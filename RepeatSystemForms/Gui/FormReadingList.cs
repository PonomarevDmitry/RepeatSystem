...
{
    public partial class FormReadingList : Form, IFormConfiguration
    {
        #region Конструктор.
        ...
        private bool RefreshList(out DataTable dataTable, out string message)
        {
            ...
            try
            {
                if (Config.Session.ReadingUtils.GetTable(out dataTable, out message))
                {
                    result = true;
                }
            }
        ...
        private void bNView_Click(object sender, EventArgs e)
        {
            ...
            if (id.HasValue)
            {
                Reading obj = Config.Session.ReadingUtils.GetById(id.Value);
        ...
        private void bNDelete_Click(object sender, EventArgs e)
        {
            ...
            if (id.HasValue)
            {
                ...
                if (MessageBox.Show(this, "Вы действительно хотите удалить чтение?", "Предупреждение", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.OK)
                {
