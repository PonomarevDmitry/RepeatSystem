...
{
    public class BookTagCompositionUtils
    {
        private Session session { get; set; }

        internal BookTagCompositionUtils(Session session)
        {
            this.session = session;
        }

        public const string tableName = "BOOK_TAGS";
        ...
        private const string tableFieldID_TAG = "ID_TAG";
        ...
        public bool GetTableForBook(int idBook, out DataTable table, out string message)
        {
            ...
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.AppendLine(string.Format("select a.*, b.{0}", TagUtils.tableFieldNAME));
            sqlStr.AppendLine(string.Format("from {0} AS a", TableNameWithScheme));
            sqlStr.AppendLine(string.Format("INNER JOIN {0} AS b ON (a.{1} = b.{2})", TagUtils.TableNameWithScheme, tableFieldID_TAG, TagUtils.tableFieldID));
            sqlStr.AppendLine(string.Format("where a.{0} = @p_id_book", tableFieldID_BOOK));
            sqlStr.AppendLine(string.Format("order by {0}", TagUtils.tableFieldNAME));
